...
async function publicKey(): Promise<string> {
  ...
  const { data, error } = await supabase.functions.invoke("push-subscribe", { body: { action: "key" } });
...
async function save(sub: PushSubscription) {
  const { error } = await supabase.functions.invoke("push-subscribe", { body: { subscription: sub.toJSON() } });
...
export async function disablePush(): Promise<PushState> {
  ...
  const sub = await r?.pushManager.getSubscription();
  if (sub) {
    await supabase.functions.invoke("push-subscribe", { body: { action: "remove", endpoint: sub.endpoint } }).catch(() => {});
...
export async function syncPushOnLoad(): Promise<PushState> {
