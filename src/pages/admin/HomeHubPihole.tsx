      ...
      {data && (
        <>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
            ...
            <StatCard label="Percent Blocked" value={`${data.percentBlocked}%`} icon={ListFilter} />
            ...
            <div className="h-64">
              <ResponsiveContainer width="100%" height="100%">
                <LineChart data={data.queriesOverTime}>
                  ...
                  <Line type="monotone" dataKey="allowed" stroke="#4ade80" strokeWidth={2} dot={false} name="Allowed" />
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <div className="bg-white/[0.03] border border-white/[0.06] rounded-2xl p-4 sm:p-6">
              <div className="flex items-center justify-between mb-4">
                ...
                <ExportButton data={filteredBlocked} filename="pihole-blocked" columns={[{ key: "domain", label: "Domain" }, { key: "count", label: "Count" }]} />
              </div>
              ...
              <div className="divide-y divide-white/[0.06]">
                {filteredBlocked.map((d) => (
                  <div key={d.domain} className="flex items-center justify-between py-2.5">
                    ...
                    <span className="text-sm text-white/40 tabular-nums shrink-0 ml-3">{d.count.toLocaleString()}</span>
                  </div>
                ))}
              </div>
            </div>
            ...
            <div className="bg-white/[0.03] border border-white/[0.06] rounded-2xl p-4 sm:p-6">
              <div className="flex items-center justify-between mb-4">
                ...
                <ExportButton data={filteredPermitted} filename="pihole-permitted" columns={[{ key: "domain", label: "Domain" }, { key: "count", label: "Count" }]} />
              </div>
              <div className="relative mb-3">
                <Search className="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-white/30" />
