...
export function PartnerMark({
  ...
  const mode = stare ? " gm-watching" : animated ? "" : " gm-still";
  const look = stare ? { transform: `translate(${stare.x}px, ${stare.y}px)` } : undefined;
  const lid = stare ? { transform: "translateY(-5.2px)" } : undefined;
  // Lean the axis toward the cursor: left of the globe tilts left, right tilts right.
  const tilt = stare ? { transform: `rotate(${(TILT + (stare.x / REACH.x) * 14).toFixed(1)}deg)` } : undefined;
      ...
      <g mask={`url(#gm${uid})`}>
        <g clipPath={`url(#gb${uid})`}>
          <g className="gm-tilt" style={tilt}>
            <ellipse className="gm-grid" cx={C.x} cy={C.y} rx="8.5" ry={R} />
